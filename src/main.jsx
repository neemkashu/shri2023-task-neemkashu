...
const ScrollPositions = TABS_KEYS.reduce((accum, key) => {
  accum[key] = 0;
  return accum;
}, {})

function TabPanel ({scrollPosition, activeTab, forwardRef }) {
  React.useEffect(() => {
      forwardRef.current.scrollLeft = scrollPosition;
  }, [activeTab])
  return (
      <div
      ref={forwardRef}
...
const DevicesMemiozed = React.memo( function ({activeTab}) {
  const panelContainer = React.useRef();
  const panelWrapper = React.useRef();
  const scrollPositions = React.useRef(ScrollPositions);
  ...
  React.useEffect(() => {
      setHasRightScroll(panelContainer.current.scrollWidth > panelContainer.current.clientWidth)
  }, [activeTab])

  return (
      <div ref={panelWrapper} className="section__panel-wrapper">
          <TabPanel scrollPosition={scrollPositions.current[activeTab]} forwardRef={panelContainer} activeTab={activeTab}/>
          {hasRightScroll &&
...
function DevicePanel () {
  ...
  return (
      <section className="section main__devices">
          ...
          <DevicesMemiozed activeTab={activeTab} />
      </section>
  );
}
